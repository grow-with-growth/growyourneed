...
import { CONTACT_HEADER_BUTTONS, CONTACT_SUB_MODULE_SIDEBAR_CONFIG } from '../../constants';
...
export const ContactLeftSidebar: React.FC<ContactLeftSidebarProps> = ({ isOpen, activeSubModuleKey }) => {
  ...
  const currentSubModuleConfig = activeSubModuleKey
    ? CONTACT_SUB_MODULE_SIDEBAR_CONFIG[activeSubModuleKey]
    : null;

  const sidebarTitle = currentSubModuleConfig?.title || "Contact Menu";
  const actions = currentSubModuleConfig?.actions || (CONTACT_HEADER_BUTTONS.length > 0 && CONTACT_SUB_MODULE_SIDEBAR_CONFIG[CONTACT_HEADER_BUTTONS[0].key]?.actions) || [];
      ...
      {actions.map((action, index) => (
        <button
          ...
          className="w-full flex items-center justify-start text-left px-3 py-2.5 sm:px-4 sm:py-3
                     bg-slate-700/60 hover:bg-teal-600/70 rounded-md text-slate-200 hover:text-white
                     transition-all duration-200 border border-slate-600/80 hover:border-teal-500
                     ...
                     focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-opacity-60 text-xs sm:text-sm"
        >
          {action}
        </button>
      ))}
